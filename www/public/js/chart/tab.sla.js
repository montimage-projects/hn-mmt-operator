               ...
               switch (name) {
                  case "limit_gtp": {
                     const query = [
                        {
                           $match: {
                              //must concerning the current slice
                              slices: sliceId,
                              //must have TEIDs
                              "114": {$exists: true}
                           }
                           ...
                           $project: {
                              "teidLen" : {$size: "$" + GTP.TEIDs.id}
                           }
                        }, {
                           $match: {
                              //it is violated if it pass the given threshold
                              teidLen: threshold
                           }
                        }, {
                           $count: "teidLen"
                  ...
                  case "isolation_access": {
                     const query = [
                        {
                           $group: {
                              //group by the same timestamp
                              _id: "$" + COL.TIMESTAMP.id,
                              //total flows
                              total: { $sum: 1 },
                              //number of non-isolation flows
                              nonIsolate: {
                                 $sum: {
                                    $cond: {
                                       if: {
                                          $and: [
                                             //must concerning the current slice
                                             { $in: [sliceId, "$slices"] },
                                             //must connecting to another slice
                                             { $lt: [1, { $size: "$slices" }] }]
                                       },
                                       then: 1,
                                       else: 0
                           ...
                           $project: {
                              //get percentage of nonIsolate per total flows
                              violation: { $multiply: [100, { $divide: ["$nonIsolate", "$total"] }] }
                           }
                        }, {
                           $match: {
                              //it is violated if it pass the given threshold
